...
## 2. The Abstraction Layer: `src/quant_core/interfaces.py`

The heart of this architecture is the `src/quant_core/interfaces.py` file, which defines a set of abstract base classes (ABCs). These interfaces act as a formal "contract" that any market-specific implementation must adhere to.
...
All market adapters are located in the `src/quant_core/market_adapters/` directory, with each market having its own subdirectory (e.g., `src/quant_core/market_adapters/ibkr/`).
...
1.  **Create a New Directory**:
    Create a new subdirectory under `src/quant_core/market_adapters/`, for example, `src/quant_core/market_adapters/new_market/`.

2.  **Implement the Interfaces**:
    Inside this new directory, create Python classes that inherit from and implement the methods defined in `src/quant_core/interfaces.py`.
