...
## 1. The Abstract Framework (`src/quant_core/interfaces.py`)

The foundation of the entire system is the set of abstract base classes in `src/quant_core/interfaces.py`. These interfaces define a "contract" for what a market implementation must provide:
...
The first concrete implementation of the abstract framework is the IBKR Market Adapter, located in `src/quant_core/market_adapters/ibkr/`. This adapter contains all the logic necessary to connect to, fetch data from, and execute trades with Interactive Brokers.

### IBKR Connection (`src/quant_core/market_adapters/ibkr/connection.py`)
...
### IBKR Data Loader (`src/quant_core/market_adapters/ibkr/data_loader.py`)
...
### IBKR Execution Handler (`src/quant_core/market_adapters/ibkr/execution.py`)
...
## 3. The Market-Agnostic Safety Layer (`src/quant_core/execution.py`)
