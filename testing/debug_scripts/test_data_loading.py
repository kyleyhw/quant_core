...
# Add project root to path (this file lives at testing/debug_scripts/, so go up two)
...
class TestDataLoading(unittest.TestCase):
    ...
    @patch(
        ...
        read_data="Price,Close,Close\nTicker,PEP,KO\n2023-01-01,100,50\n",
    )
    def test_get_available_assets_pair(
        self, mock_file: MagicMock, mock_get_files: MagicMock
    ) -> None:
        ...
        with patch("pandas.read_csv") as mock_read_csv:
            ...
            mock_df = pd.DataFrame(
                columns=pd.MultiIndex.from_tuples(
                    [("Close", "PEP"), ("Close", "KO")], names=["Price", "Ticker"]
                )
            )
    ...
    def test_get_available_assets_single(
        self, mock_file: MagicMock, mock_get_files: MagicMock
    ) -> None:
